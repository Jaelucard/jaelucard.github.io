...
# A duration needs 个月 ("4-6个月"); a bare 月 is a calendar month ("3-6月", "2027年3月至8月").
# "N个月内" is a time window and 试用期 a probation period, not the internship length. English
# numbers must stand alone, so "RMB 8000 monthly" and "within 2 months" are not durations.
_EN_NUMBER = r"(?<![\d,.])(\d{1,2})"
_DURATION_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    (re.compile(rf"(?<!\d)(?<!试用期){NUM}\s*(?:个月)?{DASH}{NUM}\s*个月(?!\s*内){_NOT_START}"), "range"),
    (re.compile(rf"{_EN_NUMBER}\s*(?:-|~|to)\s*(\d{{1,2}})\s*months?\b", re.IGNORECASE), "range"),
    (re.compile(rf"(?:至少|最少|不少于|不低于)\s*(?:实习)?\s*{NUM}\s*个月"), "minimum"),
    (re.compile(rf"(?<!\d)(?<!试用期){NUM}\s*个月\s*(?:及)?以上"), "minimum"),
    (re.compile(r"(?:minimum|at\s+least)\s*(?:of\s*)?(\d{1,2})\s*months?", re.IGNORECASE), "minimum"),
    (re.compile(rf"{_EN_NUMBER}\s*months?\s*(?:minimum|or\s+(?:more|longer)|\+)", re.IGNORECASE), "minimum"),
    (re.compile(r"(?:至少|最少|不少于)\s*(?:实习)?\s*半年|半年(?:及)?以上"), "half_year"),
    (re.compile(rf"(?:固定|为期)\s*{NUM}\s*个月"), "fixed"),
    (re.compile(rf"(?<!within ){_EN_NUMBER}[- ]months?\b", re.IGNORECASE), "fixed"),
]
...
_DAYS_PATTERNS = [
    ...
    re.compile(r"(?<!\d)(\d)\s*(?:-|~|to)\s*\d\s*days?\s*(?:a|per|/)\s*week", re.IGNORECASE),  # range: its minimum
    re.compile(r"(?:at\s+least\s*)?(?<!\d)(\d)\s*days?\s*(?:a|per|/)\s*week", re.IGNORECASE),
]
...
# Two months offered ("3月或4月入职", "1-3月入职"): no single start to compare with.
_TWO_START_MONTHS = re.compile(
    rf"{MONTH}\s*月?\s*(?:-|~|～|至|到|或|/)\s*{MONTH}\s*月(?:份)?\s*(?:初|中|底)?\s*(?:起)?\s*(?:可)?\s*(?:入职|到岗)"
)


def start_month(text: str) -> tuple[int | None, int] | None:
    """(year or None, month) of a stated start, or None."""
    if _TWO_START_MONTHS.search(text):
        return None
...
def cross_check(text: str, extracted: ExtractedJob) -> dict[str, str]:
    ...
    if duration is not None:
        ...
        if low != extracted.duration_min_months.value:
            notes["duration_min_months"] = f"{said}; the extraction has {_shown(extracted.duration_min_months.value)}."
        if high is not None and high != extracted.duration_max_months.value:
            notes["duration_max_months"] = f"{said}; the extraction has {_shown(extracted.duration_max_months.value)}."
    days = days_per_week(text)
    if days is not None and days != extracted.days_per_week_min.value:
        notes["days_per_week_min"] = f"The posting text says {days} days a week; the extraction has {_shown(extracted.days_per_week_min.value)}."
    ...
    if start is not None:
        ...
        if current is None or current.month != month or (year is not None and current.year != year):
            notes["start_date"] = f"The posting text mentions a start in {stated}; the extraction has {_shown(current)}."
    return notes
